...
main = do
    ...
    defaultMain [
        bgroup "training" [ bench "20"  $ nf (train tokens) 20
                          ],
        bgroup "encode"   [ bench "taylorswift"  $ nf (encodeBS ms) inputBSSwift
                          , bench "shakespeare"  $ nf (encodeBS ms) inputBSShakespeare
                          ],
        bgroup "decode"   [ bench "taylorswift"  $ nf (decodeBS vocab) encodedSwift
